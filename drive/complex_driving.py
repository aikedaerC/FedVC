...
class ComplexDrivingData(Dataset):
    def __init__(self, file_path, device, dataidxs=None, 
        ...
        self.crop_size = tuple(crop_size) if crop_size else crop_size
    ...
    def _gen_pseudo(self, value):
        bins = np.linspace(-1, 1, 21) 
        pseudo_label = np.digitize(value, bins) - 1
        return pseudo_label

    def _load_truncated_data(self): 
        if self.imgs is None:
            with np.load(self.data_file) as data:
                self.filenames = data['file_names']
                self.ground_truths = data['ground_truths']
                self.pseudo_truths = np.array([self._gen_pseudo(e) for e in self.ground_truths])
            
            if self.dataidxs is not None:
                self.filenames = self.filenames[self.dataidxs]
                self.ground_truths = self.ground_truths[self.dataidxs]
                self.pseudo_truths = self.pseudo_truths[self.dataidxs]

    def __len__(self):
        return self.ground_truths.shape[0]

    def __getitem__(self, index):
        img_filename = os.path.join("C:\\Users\\aikedaer\\Desktop\\NonIIDbench\\datasets", self.filenames[index])
        ...
        pseudo_label = self.pseudo_truths[index]

        return img, pseudo_label, steering_angle
